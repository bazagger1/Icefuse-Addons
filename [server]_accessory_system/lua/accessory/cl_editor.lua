...
File Path:   addons/[server]_accessory_system/lua/accessory/cl_editor.lua
	...
	_SH_ACC_EDITOR = frame
		...
		body:GetCanvas().Paint = function(me, w, h)
			...
			local lookup = {
				...
				chest = "SH_SLOT_CHEST",
				back = "SH_SLOT_BACK",
				pelvis = "SH_SLOT_PELVIS",
			}

			local lbl = self:Label("Valid: pets, head, neck, eyes, mouth, chest, back, pelvis", "SH_ACC.MediumB", C"text", body)
			lbl:Dock(TOP)
			lbl:DockMargin(0, 0, 0, m5)
			...
			export.DoClick = function()
				...
				MsgC(tx, string.format([[local a = New("%s", {name = "%s", price = %d, slot = %s, mdl = "%s"})%s]] .. "\n\n",
					...
					(sliders.Scale ~= 1 and " a.scale = " .. math.Round(sliders.Scale, 2) .. "\n" or "")
				))
